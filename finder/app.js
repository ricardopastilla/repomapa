...
// divLoading.style.display = "flex"; //Mostrar
// divLoading.style.display = "none"; //Ocultar
...
function createPopup(currentFeature) {
    ...
    new mapboxgl.Popup({
        closeOnClick: true
    }).setLngLat(currentFeature.geometry.coordinates).setHTML('<h3>' + currentFeature.properties[config.popupInfo] + '</h3>').addTo(map);
}
    ...
    filterForm.addEventListener('change', function() {
        ...
        if (geojCheckboxFilters.length === 0 && geojSelectFilters.length === 0) {
            ...
            geojCheckboxFilters.forEach((filter) => {
                geojsonData.features.forEach((feature) => {
                    if (feature.properties[filter[0]].includes(filter[1])) {
                        if (filteredGeojson.features.filter(
                                (f) => f.properties.id === feature.properties.id, ).length === 0) {
                            filteredGeojson.features.push(feature);
                        }
                    }
                });
            ...
            geojsonData.features.forEach((feature) => {
                ...
                if (selected === true && filteredGeojson.features.filter(
                        (f) => f.properties.id === feature.properties.id, ).length === 0) {
                    filteredGeojson.features.push(feature);
                }
            });
...
map.on('moveend', (ev) => {
    const moveEnd = ev.result.geometry;
    sortByDistance(map.getCenter);
});
map.on('load', () => {
    ...
    const Pob_Us_1A = ['<', ['get', 'SumaCluster'], 2];
    const Pob_Us_1B = ['all', ['>=', ['get', 'SumaCluster'], 2],
        ['<', ['get', 'SumaCluster'], 8]
    ];
    const Pob_Us_1C = ['all', ['>=', ['get', 'SumaCluster'], 8],
        ['<', ['get', 'SumaCluster'], 11]
    ];
    // colors to use for the categories
    const colors = ['#bc955c', '#1a5d4e', '#9f2241'];
        ...
        csv2geojson.csv2geojson(csvData, {
            ...
            map.addLayer({
                ...
                paint: {
                    ...
                    'circle-color': '#1a5d4e', // color of circles
